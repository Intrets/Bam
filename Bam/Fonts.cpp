...
FontInfo Fonts::loadMonospacedFont(std::string name, glm::ivec2 charDim, glm::ivec2 gridDim) {
	...
	for (int32_t y = 0; y < gridDim.y; y++) {
		for (int32_t x = 0; x < gridDim.x; x++) {
			...
			//glm::vec2 uvBot = glm::vec2(charDim.x * (x + 1), charDim.y * (y + 1)) / glm::vec2(texSize);
...
FontInfo& Fonts::getFont(Font font) {
	return fontInfos[static_cast<int32_t>(font)];
}
...
Fonts::Fonts() {
	...
	atlasSize = glm::min(maxSize, 256);

	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, atlasSize, atlasSize, 0, GL_RGBA, GL_UNSIGNED_BYTE, (void*) 0);
	...
	glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, fontAtlas.ID, 0);

	fontInfos[static_cast<size_t>(Font::ROBOTO_12)] = loadMonospacedFont("roboto_mono_12px_7x17_36x4.dds", { 7,17 }, { 36,4 });
	fontInfos[static_cast<size_t>(Font::ROBOTO_14)] = loadMonospacedFont("roboto_mono_14px_8x19_32x4.dds", { 8,19 }, { 32,4 });
	fontInfos[static_cast<size_t>(Font::ROBOTO_16)] = loadMonospacedFont("roboto_mono_16px_10x22_25x6.dds", { 10,22 }, { 25,6 });
}
