...
bool LuaActivity::applyMove(int32_t index, int32_t type) {
	if (!this->validIndex(index)) {
		...
		for (auto h : this->labelLists[index]) {
			res |= WeakReference<Activity, Activity>(gameStateRef->getActivityManager(), h).get()->applyMoveRoot(*gameStateRef, static_cast<ACTIVITY::DIR>(type), 4);
		}
		return res;
	}
}
...
bool LuaActivity::applyActivity(int32_t index, int32_t type) {
	if (!this->validIndex(index)) {
		...
		bool res = true;
		for (auto h : this->labelLists[index]) {
			res |= WeakReference<Activity, Activity>(gameStateRef->getActivityManager(), h).get()->applyActivityLocal(*gameStateRef, type);
		}
		return res;
	}
}
...
bool LuaActivity::sendMessage(int32_t index, sol::variadic_args& va) {
	if (!this->validIndex(index)) {
		...
		for (auto h : this->labelLists[index]) {
			Activity* activity = WeakReference<Activity, Activity>(gameStateRef->getActivityManager(), h).get();
